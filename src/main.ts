...
WA.onInit().then(async () => {
  ...
  const shouldTeleport = () => {
    // Admin or localhost doesn't trigger nav
    if (isAdmin || mapURL.includes("localhost")) return false;
    // Wrong mapURL doesn't trigger nav
    if (!mapURL.includes("notlog")) return false;
    // Quest unsolved doesn't trigger nav
    if (solvedNotlog !== true) return false;
    else return true;
  };

  // Teleport the player to the entry named "matrix-hub"
  if (shouldTeleport()) WA.nav.goToRoom("./matrix-hub.tmj");

  // Event listener for entering the notlog area
  WA.room.area.onEnter("notlog").subscribe(() => {
    //todo can th above shouldTeleport condition be applied here?
    //todo if SO, it can be turned into a function and used here directly, also above.
